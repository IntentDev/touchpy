...
memCopyFromSurface(void* dst, int width, int height, cudaSurfaceObject_t src, cudaStream_t stream)
{
	dim3 blockSize(16, 16, 1);
	dim3 gridSize(divUp(width, blockSize.x), divUp(height, blockSize.y), 1);
	fromSurface<T, CompType> <<<gridSize, blockSize, 0, stream>>> (dst, width, height, src);
...
template cudaError_t memCopyFromSurface<float4, float>(void* dst, int width, int height, cudaSurfaceObject_t src, cudaStream_t stream);
template cudaError_t memCopyFromSurface<float2, float>(void* dst, int width, int height, cudaSurfaceObject_t src, cudaStream_t stream);
template cudaError_t memCopyFromSurface<float, float>(void* dst, int width, int height, cudaSurfaceObject_t src, cudaStream_t stream);
template cudaError_t memCopyFromSurface<uchar4, uint8_t>(void* dst, int width, int height, cudaSurfaceObject_t src, cudaStream_t stream);
